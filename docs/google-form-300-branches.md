# ระบบรวมข้อมูล 300 สาขา + 500 หน่วยงานร่วมโครงการ ผ่าน Google Form → PostgreSQL
...
```
500 หน่วยงานนอก (ร่วมโครงการ)
        │
        │  เข้าร่วมผ่าน
        ▼
300 สาขา ──── แต่ละสาขามีหน่วยงานภายใน 1 แห่ง
        │
        │  สาขาเป็นผู้กรอก GGF ให้ทุกหน่วยงานที่ดูแล
        ▼
  1 record / 1 หน่วยงาน / 1 วัน = 800 records/วัน
```
...
```
                 300 สาขา (ผู้กรอก)
                      │
                      │  กรอก GGF ให้ 800 หน่วยงาน
                      ▼
Google Form 1 ตัว ──→ Google Sheet 1 ตัว ──→ PostgreSQL
                                                │
                                                ▼
                                            Dashboard
```

- **สาขา 300 แห่ง** เป็นผู้กรอก Google Form ให้หน่วยงานทั้งหมด
- ใช้ **Pre-filled Link** ฝังรหัสหน่วยงานไว้ → เปิดมาไม่ต้องเลือกชื่อหน่วยงาน (แนะนำ)
...
## ปัญหาชื่อหน่วยงานยาว และแนวทางแก้ไข

ชื่อหน่วยงานราชการ/สาขามักยาวมาก ทำให้ dropdown เลือกยาก เลือกผิดง่าย เช่น
...
### วิธีที่ 1: รหัสหน่วยงาน + ชื่อย่อ ใน Dropdown

```
B001 - สสจ.นครราชสีมา
B002 - สสจ.นครศรีธรรมราช
B003 - สสจ.นครสวรรค์
```

- สั้น เลือกง่าย ค้นหาจากรหัสได้
- ต้องแจกตารางรหัสให้หน่วยงาน

### วิธีที่ 2: แยก Form ตามกลุ่ม/ภาค

```
Form 1: ภาคเหนือ (70 หน่วยงาน)
Form 2: ภาคกลาง (80 หน่วยงาน)
Form 3: ภาคอีสาน (90 หน่วยงาน)
Form 4: ภาคใต้ (60 หน่วยงาน)
Form 5: หน่วยงานร่วมโครงการ กลุ่ม A-M
Form 6: หน่วยงานร่วมโครงการ กลุ่ม N-Z
```

- dropdown สั้นลง เลือกง่าย
- ดูแลแค่ 5-6 form (ไม่ใช่ 800)
- response ไหลเข้า Sheet คนละตัว → script ดึงรวมเหมือนเดิม

### วิธีที่ 3: Pre-filled Link (แนะนำ)

สร้าง link สำหรับแต่ละหน่วยงาน โดยฝังรหัสหน่วยงานไว้แล้ว → เปิดมาไม่ต้องเลือก

```
https://docs.google.com/forms/d/FORM_ID/viewform?usp=pp_url&entry.123456=B001
```

- ใช้ form เดียว ดูแลง่าย
- หน่วยงานไม่ต้องเลือกชื่อเอง → ลดความผิดพลาด
- สร้าง 800 link อัตโนมัติด้วย script ได้

### เปรียบเทียบแนวทาง

| วิธี | ข้อดี | ข้อเสีย |
|------|-------|---------|
| **รหัส + ชื่อย่อ** | ง่ายสุด ใช้ form เดียว | ต้องแจกตารางรหัส |
| **แบ่ง form ตามกลุ่ม** | dropdown สั้น ดูแลง่าย | ดูแล 5-6 form |
| **Pre-filled link** | ไม่ต้องเลือกเลย ผิดพลาดน้อยสุด | ต้องสร้าง 800 link |

> **แนะนำวิธีที่ 3 (Pre-filled Link)** เพราะใช้ form เดียว ดูแลง่าย และหน่วยงานไม่ต้องเลือกชื่อเอง ลดความผิดพลาดได้มากที่สุด

---

## ทำไมไม่ใช้ 800 Forms / 800 Sheets

| | 800 Forms | 1 Form + Pre-filled Link |
|---|---|---|
| **ดูแล** | แก้ 800 ที่ | แก้ที่เดียว |
| **ดึงข้อมูล** | เรียก API 800 ครั้ง | เรียกครั้งเดียว |
| **Rate limit** | เสี่ยงโดน | ไม่มีปัญหา |
| **Permission** | จัดการ 800 ชุด | จัดการชุดเดียว |
| **Sync → Postgres** | script ซับซ้อน | script ง่ายมาก |
| **ชื่อหน่วยงานผิด** | เป็นไปได้ | ไม่มีปัญหา (ฝังไว้ใน link) |
...
### ประมาณการข้อมูลของโครงการนี้

| รายการ | จำนวน |
|--------|------|
| ผู้กรอกข้อมูล | 300 สาขา |
| หน่วยงานภายใน | 300 แห่ง |
| หน่วยงานนอก (ร่วมโครงการ) | 500 แห่ง |
| รวมหน่วยงาน | 800 แห่ง |
| ระยะเวลา | 92 วัน (1 พ.ค. - 31 ก.ค. 2569) |
| ความถี่ | 1 record / 1 หน่วยงาน / 1 วัน |
| Records ต่อวัน | 800 records |
| **รวม records** | **73,600 records** (800 × 92) |

> 73,600 records ถือว่าน้อยมากสำหรับทั้ง Google Sheet (limit 1M แถวที่ 10 col) และ PostgreSQL (ไม่จำกัด)
> ทั้งสองรองรับได้สบาย ไม่ต้องกังวล
